...
class Tracer(var sim: Simulator) {
    ...
    fun traceAddError(err: SimulatorError) {
        this.tr.trace.add(Trace(false, false, "", Array(0) { 0 }, MachineCode(0), 0, 0, Memory(), error = err))
    }
    ...
    fun traceStart() {
        ...
        if (this.twoStage) {
            this.tr.trace.add(Trace(didBrach(), didJump(), getecallMsg(), getRegs(), if (!sim.isDone()) sim.getNextInstruction() else MachineCode(0), this.tr.traceLine, sim.getPC(), sim.state.mem.clone()))
            this.tr.traceLine++
        }
...
//            }
    ...
    fun getSingleTrace(line: Int): Trace {
        ...
        return Trace(didBrach(), didJump(), getecallMsg(), getRegs(), mc, line, sim.getPC(), sim.state.mem.clone())
    }
